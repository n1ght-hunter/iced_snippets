...
pub struct Container<'a, Message, Handle, Renderer = iced::Renderer>
where
...
impl<'a, Message, Handle, Renderer> Container<'a, Message, Handle, Renderer>
where
    Renderer: iced::advanced::Renderer,
    Renderer::Theme: StyleSheet,
{
    /// Creates an empty [`Container`].
    pub fn new<T, B>(content: T, background_image: Option<B>) -> Self
    where
...
impl<'a, Message, Handle, Renderer> Widget<Message, Renderer>
    for Container<'a, Message, Handle, Renderer>
where
...
impl<'a, Message, Handle, Renderer> From<Container<'a, Message, Handle, Renderer>>
...
where
    ...
    Handle: Clone + Hash + 'a,
{
    fn from(column: Container<'a, Message, Handle, Renderer>) -> Element<'a, Message, Renderer> {
        Element::new(column)
    }
}
